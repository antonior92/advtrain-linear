...
if __name__ == "__main__":
    ...
    for n_train in [100,]:
        ...
        params, info = lin_advregr(X_train, y_train, adv_radius=adv_radius, verbose=True, max_iter=1000, p=np.inf, method='w-ridge')
        ...
        # Compare with cvxpy
        start_time = time.time()
        paramscg, info = lin_advregr(X_train, y_train, adv_radius=adv_radius, verbose=True, p=np.inf, max_iter=1000, method='w-cg')
        exec_time = time.time() - start_time
        df = df.append({
            'method': 'cg',
            'n_params': n_params,
    ...
    plt.xlabel('time (s)')
    plt.yscale('log')
    plt.legend(title='', labels=['ours', 'cg'])
